...
void init() {
    ...
    initialisation("lapin.obj",&nbSommet,&nbFacet);
    

    coordonnees = new GLfloat[3*nbSommet];
    indices = new GLuint[3*nbFacet];
    float* bbox = new float[3*8];
    coordbb=new GLfloat[6];

    lecture("lapin.obj",nbSommet,nbFacet,coordonnees,indices,bbox);

    boundingbox(bbox,coordbb);

    float centre[3]={bbox[5]-bbox[2],bbox[4]-bbox[1],bbox[3]-bbox[0]};

    programID = LoadShaders("../Shaders/TransformVertexShader.vert", "../Shaders/SimpleFragmentShader.frag");


    glGenBuffers(2, vboid);
    glBindBuffer(GL_ARRAY_BUFFER, vboid[0]);trans_initial = glm::translate(glm::mat4(1.0f), glm::vec3(-0.25, -0.25, -0.25));
    glBufferData(GL_ARRAY_BUFFER, 3 * nbSommet * sizeof(float), coordonnees, GL_STATIC_DRAW);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 3 * nbFacet * sizeof(GLuint), indices, GL_STATIC_DRAW);
    ...
    trans_initial = glm::translate(glm::mat4(1.0f), glm::vec3(-centre[2],-centre[1],-centre[0]));
...
void Display(void) {
    ...
    glDrawElements(GL_TRIANGLES, 3 * nbFacet, GL_UNSIGNED_INT, NULL);
