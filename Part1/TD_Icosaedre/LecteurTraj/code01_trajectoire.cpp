...
static GLfloat coordonnees_ico[36] =
    {
        -X, 0, Z,
        X, 0, Z,
        -X, 0, -Z,
...
void init() {
    ...
    GLfloat* coordonnees2=new GLfloat[36*NbAtoms];
    GLfloat* couleurs2=new GLfloat[36*NbAtoms];
    ...
    for (int i = 0; i < NbAtoms; i++)
    {
        for (int j = 0; j < 12; j++)
        {
            for (int k = 0; k < 3; k++)
            {
                coordonnees2[(i * 36) +(j*3)+k] = coordonnees_ico[(j*3)+k];
                
                
                
                
                couleurs2[(i * 36) + (j*3)+k] = couleurs[(i*3)+k];
            }
        }
    }
   

    GLuint* indices2=new GLuint[60*NbAtoms];
    for (int i = 0;i<NbAtoms;i++){
        for (int j = 0;j<60;j++)
        {
            indices2[(60*i)+j]=indices_ico[j]+(12*i);
        }
    } 
    for(int i = 0; i < 60; i++){
        if (i%3==0){
    ...
    // glBufferData(GL_ARRAY_BUFFER, 3 * NbAtoms * 12 * sizeof(float), coordonnees2, GL_DYNAMIC_DRAW);
    // glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void *) 0);
    // glEnableVertexAttribArray(0);

    // glBindBuffer(GL_ARRAY_BUFFER, vboID[1]);
    // glBufferData(GL_ARRAY_BUFFER, 3 * NbAtoms * 12 * sizeof(float), couleurs2, GL_STATIC_DRAW);
    // glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, (void *) 0);
    // glEnableVertexAttribArray(1);

    // glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vboID[2]);
    // glBufferData(GL_ELEMENT_ARRAY_BUFFER, 3 * 20 * NbAtoms * sizeof(GLuint), indices2, GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, vboID[0]);
    glBufferData(GL_ARRAY_BUFFER, 3 * 12 * sizeof(float), coordonnees_ico, GL_DYNAMIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void *) 0);
    glEnableVertexAttribArray(0);

    glBindBuffer(GL_ARRAY_BUFFER, vboID[1]);
    glBufferData(GL_ARRAY_BUFFER, 3  * 12 * sizeof(float), couleurs2, GL_STATIC_DRAW);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, (void *) 0);
    glEnableVertexAttribArray(1);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vboID[2]);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 3 * 20  * sizeof(GLuint), indices_ico, GL_STATIC_DRAW);
...
void Display(void) {
    ...
    //glDrawArrays(GL_POINTS, 0, NbAtoms*36);
